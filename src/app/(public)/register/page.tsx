...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register({
        nama: name,                        // ← "nama" bukan "name"
        email,
        password,
        password_confirmation: confirmPassword,  // ← wajib ada
      })
    } catch (err: unknown) {
  ...
  return (
    <AuthLayout>
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-text">Daftar Akun Baru</h2>
        <p className="text-muted text-sm mt-2">Bergabunglah bersama ribuan warga lainnya</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-semibold text-text mb-1">Konfirmasi Password</label>
          <input
            type={showPassword ? 'text' : 'password'}
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-primary text-white font-semibold py-3 rounded-xl hover:bg-primary-hover transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-4"
        >
          {isLoading ? 'Memproses...' : (
            <>
              <UserPlus size={18} />
